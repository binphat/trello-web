...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMember }) {
  ...
  // Đoạn này lấy activeBoard từ redux ra để mục đích lấy được toàn bộ thông tin những thành viên của cái board thông qua field : FE_allUsers
   ...
   * FE_CardMembers chứa đầy đủ thông tin của User để hiển thị ra ngoài giao diện, bởi mặc định trong
   * card chỉ lưu dám ID của User thôi (memberIds)
   */
  // const FE_CardMembers = board?.FE_allUsers?.filter(users => cardMemberIds.includes(users._id))
  ...
  const handleUpdateCardMember = (user) => {
    ...
    // Action là xóa khỏi card (REMOVE) hoặc thêm vào card (ADD)
    const incomingMemberInfo = {
      userId: user._id,
      action: cardMemberIds.includes(user._id) ? CARD_MEMBER_ACTIONS.REMOVE : CARD_MEMBER_ACTIONS.ADD
    }
    // console.log('🚀 ~ handleUpdateCardMember ~ incomingMemberInfo:', incomingMemberInfo)
